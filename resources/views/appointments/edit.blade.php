                    ...
                    <form method="POST" action="{{ route('appointments.update', $appointment) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="equipment_id" id="equipment_id"
                                ...
                                @foreach ($equipment as $item)
                                    <option value="{{ $item->id }}" {{ old('equipment_id', $appointment->equipment_id) == $item->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" name="scheduled_date" id="scheduled_date" 
                                value="{{ old('scheduled_date', $appointment->scheduled_date->format('Y-m-d\TH:i')) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="description" rows="4"
                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">{{ old('description', $appointment->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                ...
                                placeholder="Add any additional notes">{{ old('notes', $appointment->notes) }}</textarea>
                            @error('notes')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
